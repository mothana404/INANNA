  ...
  const FooterLink = memo(({ to, children }) => (
    ...
    >
      <span className="relative">
        {children}
        <span
          className="absolute -left-3 sm:-left-4 rtl:-right-3 rtl:sm:-right-4 rtl:left-auto top-1/2 transform -translate-y-1/2 
            ...
            <FooterSection title={t("footer.quickLinks.title")}>
              <ul className="space-y-2 sm:space-y-3">
                {quickLinks.map((link, index) => (
                  <li key={index}>
                    <FooterLink to={link.path}>{t(link.label)}</FooterLink>
                  </li>
                ))}
              </ul>
            </FooterSection>
